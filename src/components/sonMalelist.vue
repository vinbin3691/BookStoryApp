  ...
  <div class="mysonmale">
    ...
    <ul class="malerem"   v-if="showmaleImg">
      <li @click.prevent="sonmalehotClick(malehotbook[0]._id)">
        <a>
          <div class="maleImg">
            <!-- <img :src="` /api${malehotbook[0].cover} `" /> -->
      ...
      >
        <a>
          <h3>
            <span class="leftMsg">[{{listitem.minorCate}}]</span>
            <span class="rightMsg">{{listitem.title}}</span>
          </h3>
        </a>
      </li>
    </ul>
...
export default {
  ...
  created() {
    if (this.malenewbook) {
      this.axios
      .get('/api/ranking/54d430e9a8cb149d07282496')
        // .get('http://novel.kele8.cn/rank/54d430e9a8cb149d07282496')
        .then((res) => {
          console.log(res)
      ...
      this.axios
        .get('/api/ranking/5a6844f8fc84c2b8efaa8bc5')
        .then((res) => {
          console.log(res)
          var books = res.data.ranking.books
          this.malehotbook = books.slice(0, 6)
